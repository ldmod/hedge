...
def main():
  ...
  signal_index = time.strftime("%Y%m%d%H%M00", time.localtime(int(time.time()/1800)*1800+1800+3600*8))
  # 信号文件下标步进
      ...
      try:
        with open(file_path, mode='r') as file:
          ...
          for row in csv_reader:
            key = row[0]
            value = Decimal(row[1])/Decimal('2.0')
