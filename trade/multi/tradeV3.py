...
def query_bids(client, symbol, precision, logger_error):
    try:
        ...
        if depth['bids'][0] is None:
          ...
          return Decimal('-1'), Decimal('-1'), Decimal('-1')
        return (round(Decimal(depth['bids'][0][0]), precision), round(Decimal(depth['bids'][1][0]), precision), round(Decimal(depth['bids'][2][0]), precision))
...
def query_asks(client, symbol, precision, logger_error):
    try:
        ...
        if depth['asks'][0] is None:
          ...
          return Decimal('-1'), Decimal('-1'), Decimal('-1')
        return (round(Decimal(depth['asks'][0][0]), precision), round(Decimal(depth['asks'][1][0]), precision), round(Decimal(depth['asks'][2][0]), precision))
    except Exception as e:
        logger_error.info(str(e))
        return Decimal('-1'), Decimal('-1'), Decimal('-1')
...
def process_task(position_value_map, position_amount_map, api_key, api_secret, i, cfg):
  ...
  signal_dir = cfg['signal']['signal_dir']
  pred_dir = cfg['signal']['pred_dir']
  bid_multiplier = Decimal(str(cfg['bid_multiplier']))
  ask_multiplier = Decimal(str(cfg['ask_multiplier']))
  bid_multiplier2 = Decimal(str(cfg['bid_multiplier2']))
  ask_multiplier2 = Decimal(str(cfg['ask_multiplier2']))
  ...
  while(True):
    try:
      ...
      # 等待并读取交易信号
      signal_file_path = f'{base_path}{signal_dir}/{signal_index}{signal_suffix}'
      pred_file_path = f'{base_path}{pred_dir}/{signal_index}{signal_suffix}'
      logger_trade.info("------寻找信号和预测文件: " + str(signal_file_path) + " ------")
      signal_wait_count = 0

      while not os.path.exists(signal_file_path) or not os.path.exists(pred_file_path):
      ...
      alpha_map = {}
      predict_map = {}
      logger_trade.info("------开始处理信号文件: " + str(signal_file_path) + " ------")
      try:
        with open(signal_file_path, mode='r') as file:
          ...
          for row in csv_reader:
            ...
            alpha_map[key] = value
          logger_trade.info("alpha_map:"+str(alpha_map))
      except Exception as e:
        logger_error.info(str(e))
        continue

      logger_trade.info("------开始处理预测文件: " + str(pred_file_path) + " ------")
      try:
          with open(pred_file_path, mode='r') as file:
              csv_reader = csv.reader(file)
              next(csv_reader)  # 跳过表头行
              for row in csv_reader:
                  key = row[0]
                  value = float(row[1])
                  predict_map[key] = value
          logger_trade.info("predict_map:"+str(predict_map))
      except Exception as e:
          logger_error.info(str(e))
          continue
      first_real_delta={}
      ...
      for i in range(retry_count):
        ...
        for key, value in random_position_value_items:
          if key in alpha_map: # 有权重(需要交易,包含0)
            ...
            if not key in first_real_delta:
                first_real_delta[key] = real_delta
            if real_delta*first_real_delta[key] <0:
                real_delta=0
            if abs(first_real_delta[key])<15.0:
                #logger_trade.info("skip:"+str(first_real_delta[key]))
                continue
            
            positive_threshold = get_threshold(key)
            negative_threshold = positive_threshold * Decimal("-1")
            if key not in predict_map or predict_map[key] < 0:
                predict_map[key] = 0
            #
            sidx=int((1-predict_map[key])*len(cfg["tradelevels"])) if first_real_delta[key] >0 else int((predict_map[key])*len(cfg["tradelevels"]))
            sidx=min(sidx, len(cfg["tradelevels"])-1)
            tradecfg=cfg["tradelevels"][sidx]

            delay, spliter = Decimal(str(tradecfg["delay"])), Decimal(str(tradecfg["spliter"]))
            fdratio, sdratio, thdratio = Decimal(str(tradecfg["fdratio"])), Decimal(str(tradecfg["sdratio"])), Decimal(str(tradecfg["thdratio"]))

            logger_trade.info(key+"alpha:"+str(alpha_map[key])+" sidx:"+str(sidx)+ " tradecfg: " + str(tradecfg) + " predict: " + str(predict_map[key])+" frd:"+str(first_real_delta[key])+ " realdalta:"+str(real_delta))
            delta = first_real_delta[key] / Decimal(spliter)
            delta = min(real_delta, delta) if first_real_delta[key] >0 else max(real_delta, delta)
            first_divide_delta = delta * fdratio
            first_divide_delta = first_divide_delta if abs(first_divide_delta) > 2*positive_threshold else Decimal("0")
            second_divide_delta = min(delta * sdratio, delta-first_divide_delta) if first_real_delta[key] >0 else max(delta * sdratio, delta-first_divide_delta)
            second_divide_delta = second_divide_delta if abs(second_divide_delta) > 2*positive_threshold else Decimal("0")
            third_divide_delta = delta*(fdratio+sdratio+thdratio) - first_divide_delta - second_divide_delta
            third_divide_delta = third_divide_delta if abs(third_divide_delta) > 2*positive_threshold else Decimal("0")

            if (delta > positive_threshold): #买入
                logger_trade.info("------" + "开始处理: " + str(key) + "------")
                logger_trade.info("real_delta: " + str(real_delta) + " delta: " + str(delta)+ " first_real_delta:"+str(first_real_delta[key]))
                logger_trade.info("Delta为: " + str(delta)+" fd:"+str(first_divide_delta)+" sd:"+str(second_divide_delta)+" td:"+str(third_divide_delta)+" a0:"+str(alpha_value==0))
                symbol_set.add(key)
                if (i == 0):
                ...
                if (i <= delay):
                  logger_trade.info("Skip")
                  continue
                bid0, bid1, bid2 = query_bids(client, key, price_precision_map[key], logger_error)
                logger_trade.info("bid0: " + str(bid0)+" bid1:"+str(bid1)+" bid2:"+str(bid2))
                if (bid0 > 0) and (bid1 > 0) and (bid2 > 0):
                  nbid1 = bid0 * bid_multiplier
                  nbid1 = nbid1.quantize(Decimal(f'1.{"0" * price_precision_map[key]}'))
                  bid1=min(bid1, nbid1)
                  nbid2 = bid0 * bid_multiplier2
                  nbid2 = nbid2.quantize(Decimal(f'1.{"0" * price_precision_map[key]}'))
                  bid2 = min(max(nbid2, bid2), bid1)
                  logger_trade.info("nbid0: " + str(bid0)+" nbid1:"+str(bid1)+" nbid2:"+str(bid2))
                  # 平仓单
                  ...
                  if (third_divide_delta > 0):
                    trade_amount = third_divide_delta / Decimal(bid2)
                    trade_amount = trade_amount.quantize(Decimal(f'1.{"0" * quantity_precision_map[key]}'))
                ...
                if (i <= delay):
                  logger_trade.info("Skip")
                ask0, ask1, ask2 = query_asks(client, key, price_precision_map[key], logger_error)
                logger_trade.info("ask0: " + str(ask0)+" ask1:"+str(ask1)+" ask2:"+str(ask2))
                if (ask0 > 0) and (ask1 > 0) and (ask2 > 0):
                  nask1 = ask0 * ask_multiplier
                  nask1 = nask1.quantize(Decimal(f'1.{"0" * price_precision_map[key]}'))
                  ask1=max(ask1, nask1)
                  nask2 = ask0 * ask_multiplier2
                  nask2 = nask2.quantize(Decimal(f'1.{"0" * price_precision_map[key]}'))
                  ask2 = max(min(nask2, ask2), ask1)
                  logger_trade.info("nask0: " + str(ask0)+" nask1:"+str(ask1)+" nask2:"+str(ask2))
                  # 平仓单
                  ...
                  # 下3个单: 第三个单
                  if (third_divide_delta < 0):
                    trade_amount = third_divide_delta / Decimal(ask2)
                    trade_amount = trade_amount.quantize(Decimal(f'1.{"0" * quantity_precision_map[key]}'))
                    trade_amount = abs(trade_amount)
        ...
        # 输出本轮交易成交率
        if (deal_denominator > 0) and (i > 0):
          deal_ratio = round(1 - (deal_numerator/deal_denominator), 2)
          logger_trade.info("成交率:" + str(deal_ratio))
          deal_numerator = 0
...
def main():
  ...
  # 读取 YAML 配置文件
  with open('/home/ubuntu/cryptoqt/trade/multi/configV3.yaml', 'r') as file:
