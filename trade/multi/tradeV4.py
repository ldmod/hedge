...
def post_orders_in_parallel(clients, orders, logger_trade, logger_error, price_precision_map, quantity_precision_map,
                            next_tm, close_position_set, position_amount_map):
    def place_order(client, order, request_ask_bid=False):
        try:
            ...
            if abs(order_money) < get_threshold(symbol):
                # logger_error.error(f"Less than threshold, {symbol} skip!!!")
                return False

            if order_money > 0:
                if request_ask_bid:
                    bid0, bid1, bid2, bid3, bid4 = query_bids(client, symbol, price_precision_map[symbol],
                                                              logger_error)
                ...
                #reduce = "false" if symbol not in close_position_set else "true"
                reduce = "false"
                #if (symbol in close_position_set) and (trade_amount > abs(position_amount_map[key])):
                #    trade_amount = round(abs(position_amount_map[key]), quantity_precision_map[symbol])
                ...
                if request_ask_bid:
                    ask0, ask1, ask2, ask3, ask4 = query_asks(client, symbol, price_precision_map[symbol],
                                                              logger_error)
...
# Multi threads
def process_task(position_value_map, position_amount_map, api_key, api_secret, thread_index, cfg):
    # Signal file path
    ...
    clients = [UMFutures(key=api_key, secret=api_secret, proxies=cfg["proxy"], timeout=cfg["timeout"]) for _ in
               range(5)]
    # Init logger
    logfile_trade = f"./logs/alpha-trade{thread_index}.log"
    logfile_error = f"./logs/alpha-error{thread_index}.log"
    logger_trade = get_logger(f"logger_trade{thread_index}", logfile_trade)
    logger_error = get_logger(f"logger_error{thread_index}", logfile_error)
    ...
    signal_index = time.strftime("%Y%m%d%H%M00", time.localtime(int(time.time() / 900) * 900 + 900))
    ...
    signal_wait_max = 1200  # 60s*20min/2s

    while True:
        try:
            # symbols which traded this time
            symbol_set = set()
            close_position_set = set()
            real_delta_dict = {}
            clear_sets(cancel_order_id_sets)
            # signal_index='20240831114500'
                ...
                try:
                    with open(file_path, mode='r') as file:
                        ...
                        for row in csv_reader:
                            key = row[0]
                            value = round(float(row[1]) * cfg["signal"]["money_scale"], 2)  # TODO checkpoint: 10000$
                            alpha_map[key] = value
                    break
                except Exception as e:
            ...
            for i in range(retry_count):
                ...
                for order_index, order in enumerate(orders):
                    ...
                    order_id = f"{PREFIX_ORDER_ID}-{i}-{thread_index}-{order_index}-{symbol}"
                ...
                post_orders_in_parallel(clients, orders, logger_trade, logger_error, price_precision_map,
                                        quantity_precision_map, next_tm, close_position_set, position_amount_map)
            ...
            for key in gorder.smpairs.keys():
                ...
                if abs(f_value) > 5:
                    ...
                    real_value += f_position_value * abs(f_value) / f_value
                    individual_ratio = 1 - f_position_value / f_value
                    logger_trade.info(
                        f"Key: {key}, Delta money: {f_position_value}, Target money: {f_value}, Ratio: {individual_ratio}")
            if target_value > 0:
...
if __name__ == "__main__":
    ...
    position_value_maps = [copy.deepcopy(submap) for submap in submaps]
    position_amount_maps = [copy.deepcopy(submap) for submap in submaps]

    with open('./configV4.yaml', 'r') as file:
        cfg = yaml.safe_load(file)

    # process_config = cfg['processes'][0]
    # i = process_config['id']
    # api_key = process_config['api_key']
    # api_secret = process_config['api_secret']
    # process_task(position_value_map[i], position_amount_map[i], api_key, api_secret, i, cfg)
    ...
    for process_config in cfg['processes']:
        ...
        process = multiprocessing.Process(
            target=process_task,
            args=(position_value_maps[i], position_amount_maps[i], api_key, api_secret, i, cfg)
        )
