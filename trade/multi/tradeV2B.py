...
def get_threshold(symbol):
    ...
    return values.get(symbol, Decimal(6))
  ...
  while(True):
    try:
      ...
      for i in range(retry_count):
        ...
        for key, value in random_position_value_items:
          if key in alpha_map: # 有权重(需要交易,包含0)
            ...
            spliter = cfg["spliter"]
            delta = real_delta * Decimal((i // 10 + 1) / spliter)

            positive_threshold = get_threshold(key)
            negative_threshold = positive_threshold * Decimal("-1")

            first_divide_delta = delta * Decimal('0.2')
            first_divide_delta = first_divide_delta if abs(first_divide_delta) > positive_threshold else Decimal("0")

            second_divide_delta = delta * Decimal('0.4')
            second_divide_delta = second_divide_delta if abs(second_divide_delta) > positive_threshold else Decimal("0")

            third_divide_delta = delta * Decimal('0.4')
            third_divide_delta = third_divide_delta if abs(third_divide_delta) > positive_threshold else Decimal("0")
            ...
            if (delta > positive_threshold): #买入
                ...
                if (bid0 > 0) and (bid1 > 0) and (bid2 > 0):
                  ...
                  # 平仓单
                  if (alpha_value == 0 and delta < 2*positive_threshold):
                ...
                if (ask0 > 0) and (ask1 > 0) and (ask2 > 0):
                  ...
                  # 平仓单
                  if (alpha_value == 0 and delta > 2*negative_threshold):
                    trade_amount = abs(Decimal(position_amount_map[key]))
      ...
      if target_value > 0:
        ...
        logger_trade.info(f"真实成交量: {real_value}, 目标成交量: {target_value}, 成交率: {deal_ratio}")
    except Exception as e:
      logger_error.info(str(e))
      # 异常通知到钉钉
      notify(str(e))
...
def main():
  ...
  # 读取 YAML 配置文件
  with open('/home/ubuntu/cryptoqt/trade/multi/configV2B.yaml', 'r') as file:
