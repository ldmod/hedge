      ...
      for i in range(retry_count):
        ...
        for key, value in random_position_value_items:
          if key in alpha_map: # 有权重(需要交易,包含0)
            ...
            second_divide_delta = (delta - first_divide_delta) * Decimal(min(i + 15, scnt) / scnt)
            second_divide_delta = second_divide_delta if abs(second_divide_delta) > 2*positive_threshold else Decimal("0")
            third_divide_delta = delta - first_divide_delta - second_divide_delta
...
def main():
  ...
  # 读取 YAML 配置文件
  with open('/home/ubuntu/cryptoqt/trade/multi/configV2.yaml', 'r') as file:
