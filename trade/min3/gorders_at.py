...
class GenerateOrders:
    ...
    def gen_orders(self, curtm):
        ...
        for sid,item in self.smpairs.items():
            ...
            cnt_ratio=np.clip(cnt_ratio*self.trw, 0, 1.0)
            avg_money=item["avg_money"]*self.trw
            ...
            # if abs(target["target_money"]) > 1.0 and target["cr_avg"] < 1.0:
            #     exceed_ratio = 1.00 + np.clip(0.3-target["tr_avg"]*0.5, 0, 0.1)
            ...
            if sid in self.symbolidx:
                tratio=self.mb_tratio[self.symbolidx[sid]] if avg_money>0 else self.ms_tratio[self.symbolidx[sid]]
            else:
                tratio=2.0
            # tratio=self.maxr
            high_tratio = get_threshold(sid)/abs(avg_money)
            ...
            tr_ratio=bp3_mbr=bp3_msr=lsr=ls_mom=ls_merge=pred_merge=dv=0.0
            # if self.tr_map_flag : 
            #     # bps=self.bp_map[-1]
            #     bps=3
            ...
            minr = self.cfg["minr"]
            adav = 1.0
            tr_ratio = tr_ratio**self.rwp
            select_bp += self.cfg["max_shift"]
            limit_ratio=maxr
            tratio = np.clip((1.0/((tr_ratio+0.01)))*tratio, minr, limit_ratio)
            b_tratio =  tratio
            select_bp = select_bp * np.clip(tr_ratio, 0.5, 2.0)
            ...
            bu_w=self.cfg["bu_w"][0] if tr_ratio > self.cfg["sfth"] else self.cfg["bu_w"][1]
            ...
            if target["cr_avg"] > 1.05 and cnt_ratio > 0.15 and tr_ratio > self.cfg["sfth"]:
                # tratio = tratio / np.clip(tr_ratio - 0.0, 1, 1.5)
                tratio = tratio / np.clip(target["cr_avg"] + self.cfg["sucw"], 1, 1.5)
                tratio = np.clip(tratio, minr, self.maxr)
                bpw=1.0
                select_bp = select_bp / np.clip(2.0 - tr_ratio , 0.3, 1.0)
                select_bp = select_bp / np.clip(2.0 - target["cr_avg"] - 0.2, 0.3, 1.0)
            
            n_cr_avg = target["cr_avg"] * (1-cnt_ratio) + expect_completed_ratio * cnt_ratio
            cnt_cravg = n_cr_avg-np.clip(cnt_ratio*self.cfg["cnt_cravg_shift"], 0, self.cfg["cnt_cravg_shift"])
            # cnt_cravg=target["cr_avg"]
            if cnt_cravg  < 1.0 and cnt_ratio > 0.15 and tr_ratio < self.cfg["fth"]:
                tratio = tratio / np.clip(cnt_cravg-(self.cfg["ucw"]-1)*cnt_ratio, 0.3, 1.0)
                tratio = np.clip(tratio, minr, self.maxr*2.0)
                bpw=self.cfg["bpw"]
                # select_bp = select_bp / np.clip((1+(1-tr_ratio)*bpw +
                #                                   (self.cfg["ucw"]-1)*cnt_ratio*0.3), 1.0, bpw*2)
                select_bp = select_bp / np.clip((1+(1-n_cr_avg)*bpw + 
                                                  (self.cfg["ucw"]-1)*cnt_ratio*0.3), 1.0, bpw*2)
                bu_w += cnt_ratio*0.1
                # left_money += target["f_money"]*0.1
            ...
            if abs(left_money) < get_threshold(sid)*7:
                ...
                adaptive_money=avg_money*money_normw+\
    ...
    def get_tr(self, curtm, period):
        ...
        for bp in self.bp_map:
            ...
            for s1i in range(start, end, self.delta):
                ...
                trade_money=(avgmoney*self.mb_tratio)[mb_tradeflag]
                ...
                trade_money=(avgmoney*self.ms_tratio)[ms_tradeflag]
    ...
    def get_tratio(self, curtm):
        ...
        for tidx in range(start, end, tradedelta):
            ...
            for s1i in range(tstart, tend, self.delta):
                ...
                vwap, _, svwap, shigh, slow, _, bvwap, bhigh, blow, _, smoney, bmoney, _, _=getyvalues(
                    self.secdr, s1i, self.delta*self.cfg["simdc"])
    ...
    def update_tratio(self, curtm):
        ...
        self.mb_tratio=np.clip(mb_tratio, 1, 5.0)
        self.ms_tratio=np.clip(mb_tratio, 1, 5.0)
