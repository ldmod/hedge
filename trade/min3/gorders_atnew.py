...
class GenerateOrders:
    ...
    def gen_orders(self, curtm):
        ...
        for sid,item in self.smpairs.items():
            ...
            limit_ratio=maxr
            tratio = np.clip((self.trw/((tr_ratio+0.01)**self.rwp))*tratio, minr, limit_ratio)
            b_tratio =  tratio
            ...
            if target["cr_avg"] > 1.0 and cnt_ratio > 0.15 and tr_ratio > 1.0:
                tratio = tratio / np.clip(tr_ratio - 0.0, 1, 1.5)
                tratio = tratio / np.clip(target["cr_avg"] - 0.0, 1, 1.5)
                tratio = np.clip(tratio, minr, self.maxr)
            if target["cr_avg"] < 0.9 and cnt_ratio > 0.15 and tr_ratio < 1.0:
                tratio = tratio / np.clip(tr_ratio*2-self.cfg["ucw"], 0.3, 1.0)
                tratio = np.clip(tratio, minr, self.maxr*2.0)
                bpw=1.0
                select_bp = select_bp / np.clip((bpw*2.0-tr_ratio*bpw), 1.0, bpw*2)
                select_bp = select_bp / np.clip((bpw*2.0-target["cr_avg"]*bpw), 1.0, bpw*2)
    ...
    def update_tratio(self, curtm):
        ...
        self.mb_tratio=np.clip(mb_tratio, 1, 10.0)
        self.ms_tratio=np.clip(mb_tratio, 1, 10.0)
