...
def run_alpha(delta, alphafunc, tsf=None, start=20231030000000, end=20241221203000, 
              money=30000, tratio=0.1, alphaavg=alphaavg, delaymin=1, endflag=False,
              ban_hours=[],
    ...
    for min1i in range(start, end, delta):
        ...
        if not (os.path.exists(fname) and lasteddownload>=min1i+delta) :
            if endflag:
                break
        while not (os.path.exists(fname) and lasteddownload>=min1i+delta):
            time.sleep(2)
            lasteddownload=dmap.gettmsize()
            continue
        ...
        p_flag = dr["min1info_vwap"][min1i-delaymin:min1i] > dr["min1info_high"][min1i:min1i+delta]
        
        endp=np.nanmean(dr["min1info_vwap"][min1i+delta-delaymin:min1i+delta], axis=0)
        startp=np.nanmean(dr["min1info_vwap"][min1i-delaymin:min1i], axis=0)
        ...
        df["long"]=long
        df["short"]=short
        df["bprice"]=np.nanmean(dr["min1info_vwap"][min1i-delaymin:min1i], axis=0)
        df["eprice"]=np.nanmean(dr["min1info_vwap"][min1i+delta-delaymin:min1i+delta], axis=0)
...
if __name__ == "__main__":
    ...
    # parser.add_argument('--start_date', help='start_date', default=20240701000000, type=int)
    parser.add_argument('--start_date', help='start_date', default=20240831000000, type=int)
    # parser.add_argument('--start_date', help='start_date', default=20241001140000, type=int)
    ...
    path="/home/crypto/sec_klines_env/cryptoqt/smlp/model_states/infmodel/smlpv23/res"
    # path="/home/crypto/sec_klines_env/cryptoqt/smlp/model_states/infmodel/tsmlpv230/res"
    ...
    aa=run_alpha(int(conts.h1mincnt/20), partial(min15_alpha.readcsv_v2avg,
            path=path),
            path=path,
            ...
            calcw=partial(calcwtopkliqV3, ratio_limit=60, scale=4, money_limit=5000000, 
            ...
            ban_hours=dr["ban_hours_less"],
              ...
              money=10000, tratio=0.03, lb_ratio=0.0)
