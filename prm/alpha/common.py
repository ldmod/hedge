...
class FeatureModel(nn.Module):
    ...
    def testday(self, tidx):
        latestyidx=tidx-max(self.ydays()*gv["traindelay"], gv["ltdelta"]+(gv["traindelay"]-1)*self.ydays())
        # self.olitrain.infferandstats(self, latestyidx-self.ydays()*16)
        x, vf, xetr = self.oli.addx(self, tidx)
        df=self.oli.calcstats(self, latestyidx)
        rx=d2i.getrx(gv["insdict"].dd, x, tidx)
        return rx, vf, xetr
