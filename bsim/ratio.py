...
cal_minute_list = [6, 21, 36, 51]
ret_minute_list = [2, 17, 32, 47]

signal_path = "/home/crypto/signal/predv215_5w"
signal_suffix = '_book.csv'


output_directory = "/home/crypto/position/"
today = datetime.now().strftime("%Y%m%d")
csv_file_path = os.path.join(output_directory, f"{today}.csv")
# headers = ["account_balance", "long_positions_value", "long_positions", "short_positions_value", "short_positions", "index"]
headers = ["account_balance", "long_positions_value", "short_positions_value", "index"]
...
def update_positions(client: UMFutures, position_value_map, position_volume_map) -> bool:
    try:
    ...
    while True:
        ...
        if current_time.minute in ret_minute_list:
            ...
            if len(previous_position_volume_map) != 0:
                ...
                current_time = current_time - timedelta(minutes=17)
                ...
                df["vwap_before"] = np.nanmean(dr["min1info_vwap"][min1i-15:min1i-10], axis=0)
        ...
        if current_time.minute in cal_minute_list:
            ...
            update_positions(client, available_token_map, available_token_map_volume)
