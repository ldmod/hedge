...
if __name__ == "__main__":
    ...
    parser.add_argument('--dst',  help='dst', default='/home/crypto/signal/predv215_5w')
    min5_alpha_path="/home/crypto/sec_klines_env/cryptoqt/smlp/model_states/infmodel/smlpv25_2/res"
        ...
        if os.path.exists(fname):
            try:
                if not str(dr["min1info_tm"][min1i]) in ddates:
                    ...
                    min5_alpha=min15_alpha.readcsv_v2avg(dr, min1i, min5_alpha_path)
                    alpha[ban_flag.astype(bool)]=0
                    alphas.append(alpha)
                    if len(alphas)>2:
                        alpha=runalpha.alphaavg2(alphas)
                    bookw, long, short = runalpha.calcwtopkliqV3(dr, min1i, alpha.copy(),
                                        money=10000, tratio=0.05, lastbookw=lastbookw, 
                                        ratio_limit=100)
                    if int((tm % 1000000)/10000) in dr["ban_hours"]:
                        bookw[:]=0
                        short[:]=False
                        long[:]=False
                    lastbookw=bookw
