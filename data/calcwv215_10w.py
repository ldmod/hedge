...
if __name__ == "__main__":
    ...
    parser.add_argument('--dst',  help='dst', default='/home/crypto/signal/predv215_10w')
    ...
    min1i=dr["min1info_tm"].tolist().index(20240812160000)
    delta=15
        ...
        if os.path.exists(fname):
            try:
                if not str(dr["min1info_tm"][min1i]) in ddates:
                    ...
                    alphas.append(alpha)
                    if len(alphas)>2:
                        alpha=runalpha.alphaavg(alphas)
                    bookw, long, short = runalpha.calcwtopkliqnew(dr, min1i, alpha.copy(), money=100000, tratio=0.50)
                    df=pd.DataFrame({})
                    df["sid"]=dr["sids"]
                    df["bookw"]=bookw
                    df["long"]=long
                    ...
                    #retryscp(args.dst+"/"+str(tm)+"_book.csv", "/home/ubuntu/signal/predv215_10w/")
