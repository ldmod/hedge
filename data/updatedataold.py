        ...
        try:
            ...
            if savename[0]=='s':
                if symbol in g_spotsymbols:
                    ohlcv=spot_clients[cidx].klines(symbol, timeframe, limit=1500, startTime=startTime, endTime=endTime)
                else:
                    ohlcv=[]
            else:
                ohlcv = um_futures_clients[cidx].continuous_klines(symbol, "PERPETUAL", timeframe, limit=1500, startTime=startTime, endTime=endTime)
            return ohlcv
...
def updateforover():
    ...
    while True:
        ...
        if tmi > start_tm+int(msperday/2):
            ...
            if not (int(tmsecond / 100)%10 in [0,5] \
                ...
                time.sleep(1)
                continue
        print("start update to:", tools.tmu2i(start_tm), tools.tmu2i(end_tm), tools.tmu2i(tmi))
...
def redo_dumpdayh5(ss, start_tm,  end_tm,  frequency, savename):
    delta=g_mspermin if frequency == '1m' else g_mspermin*1440
    end_tm_idx=gettmidx(end_tm, frequency)
    h5path=g_h5path
    fpath=h5path+savename+".h5"
    ...
    net=tpl.add("net")
    start_tm_idx=gettmidx(start_tm, frequency)
    fixstart_tm=gettm(start_tm_idx, frequency)
    fixend_tm=gettm(end_tm_idx, frequency)
    ...
    fetchfunc=partial(updatevalue, frequency, fixstart_tm, fixend_tm, dd, savename, start_tm_idx)
    # executor = ThreadPoolExecutor(max_workers=g_clientnum)
    # rets=executor.map(fetchfunc, ss.tolist())
    # for idx, ret in enumerate(rets):
    #     print("end update info:", ss[idx], ret, frequency, tools.tmu2i(fixstart_tm), tools.tmu2i(end_tm), flush=True)
    #     if ret < 0:
    #         print("download error:", ss[idx], ret, frequency, tools.tmu2i(fixstart_tm), tools.tmu2i(end_tm), flush=True)
    #         exit(0)
    for sidx, sid in enumerate(ss):
        fetchfunc(sid)
    ...
    for key in f.keys():
        if key != "tm":
            assert f[key].shape[0] >=end_tm_idx
            f[key][start_tm_idx:end_tm_idx,:]=dd[key][:,:]
    ...
    tpl.end()
    print("redo succ", savename, frequency, tools.tmu2i(fixstart_tm), tools.tmu2i(end_tm), tpl.to_string(), flush=True)
    
    
def redo_updatedatah5():
    readuniverse(g_data)
    msperday=1000*60*60*24
    fs=readh5new(["smin1info"])
    redoidx=[]
    for idx in range(g_data["smin1info_close"].shape[0]):
        vnum=np.isfinite(g_data["smin1info_close"][idx]).sum()
        if vnum<10:
            redoidx.append(idx)
            print(idx, np.isfinite(g_data["smin1info_close"][idx]).sum())
    for f in fs:
        f.close()
    dr=g_data
    lastidx=0
    for idx in redoidx:
        if idx -lastidx < int(conts.daymincnt/2):
            continue
        tpl=tools.TimeProfilingLoop("updatedata")
        start_tm=tools.tmi2u(dr["min1info_tm"][idx])
        end_tm=tools.tmi2u(dr["min1info_tm"][idx+int(conts.daymincnt/2)])
        # redo_dumpdayh5(g_data["sids"],start_tm,  end_tm, "1m", "min1info")
        redo_dumpdayh5(g_data["sids"], start_tm, end_tm, "1m", "smin1info")
        tpl.end()
        lastidx=idx
        print("update to:", tools.tmu2i(end_tm), tpl.to_string())
