...
def tensornorm(x, eps, dim=0):
    mean=x.nanmean(dim=dim)
    std=(((x-mean)**2).nanmean(dim=dim))**0.5
...
def getsimratio(output,vf):
    if vf.sum()==0:
        ratio=torch.ones(output.shape).to(output.device)
...
class CircleTensor:
    ...
    def update(self, xdata, s):
        ...
        self.data[s-self.s:]=xdata[:]
            
    def append(self, xdata, s):
        if self.data is None:
            self.data=self.data=torch.zeros((self.maxlen,)+xdata.shape[1:], dtype=self.dtype)
            if self.cuda:
                self.data=self.data.cuda()
        assert s == self.e and xdata.shape[0] <= self.expandlen, "append error:"+str((s,self.s, self.e, xdata.shape, self.expandlen))
        if self.data.shape[0] < self.e-self.s+xdata.shape[0]:
            for i in range(0,int(self.data.shape[0]/self.expandlen-1)):
                ss=i*self.expandlen
                ee=(i+1)*self.expandlen
                self.data[i*self.expandlen:(i+1)*self.expandlen]=self.data[(i+1)*self.expandlen:(i+2)*self.expandlen]
                #torch.cuda.empty_cache()
            self.s+=self.expandlen
        for i in range(xdata.shape[0]):
            self.data[self.e-self.s]=xdata[i]
            self.e+=1
    ...
    def getdata(self, ss, ee=0):
        ...
        return self.data[ss-self.s:ee-self.s]
    def __getitem__(self, ss, ee=0):
        return self.getdata(ss, ee)
...
class StatIc:
    ...
    def printstat(self, dur, begin, end, metric):
        ...
        flog("stats ic info",self.name,dur,"begin:", gv["ts"][begin], "end:", gv["ts"][end], pstr)
        ...
        if gv["printstd"]:
            ...
            pstr.update(stdd)
        flog("stats ic info tidx ",self.name,dur,"begin:", gv["ts"][begin], "end:", gv["ts"][end], pstr)
    ...
    def update(self, metrics, nowidx):
        ...
        lastyeardate=inttime2date(gv["ts"][self.lastyearidx],"%Y-%m-%dT%H:%M:%S")
        lastmonthdate=inttime2date(gv["ts"][self.lastmonthidx],"%Y-%m-%dT%H:%M:%S")
        last3monthdate=inttime2date(gv["ts"][self.last3monthidx],"%Y-%m-%dT%H:%M:%S")
        lastdaydate=inttime2date(gv["ts"][self.lastdayidx],"%Y-%m-%dT%H:%M:%S")
        
        nowdt=inttime2date(gv["ts"][nowidx],"%Y-%m-%dT%H:%M:%S")
...
class OlInffer:   
    ...
    def inffer(self, tmodel, tidx):
        tmodel.eval()
        output, vf, hidden = tmodel.pred(tidx)
        tmodel.train()
        #torch.cuda.empty_cache()
        return output.cpu(), vf.cpu(), hidden.cpu()
    def infferandstats(self, tmodel, tidx):
        self.addx(tmodel, tidx)
        self.addy(tmodel, tidx)
    ...
    def addx(self, tmodel, tidx):
        x, vf, hidden = self.inffer(tmodel, tidx)
        self.xx[tidx]=x.cpu()
        self.vfs[tidx]=vf.cpu()
        #torch.cuda.empty_cache()
        return x.cpu(), vf.cpu(), hidden.cpu()
    def addy(self,tmodel, tidx):
        if tidx in self.xx:
            y=tmodel.getyy(tidx)
            self.yy[tidx]=y.float().cpu()
            return y.cpu()
    def calcstats(self, tmodel, tidx):
        if not (tidx in self.xx and tidx in self.yy):
            flog("day:",tidx, gv["ts"][tidx], "tidx:",tidx,":"+self.name+" x y incomplete")
            return

        lossres=tmodel.calcloss(tidx, self.xx[tidx].to(tmodel.device), self.yy[tidx].to(tmodel.device), self.vfs[tidx].to(tmodel.device))
        df=dict(date=[gv["ts"][tidx]], tidx=[tidx], tiddur=tidx%gv["insperday"])
        for key in lossres.keys():
            df[key]=[lossres[key].detach().item()]
        x, vf, y=self.xx[tidx], self.vfs[tidx], self.yy[tidx]
        # df.update(dict(xstd=x.float().std().item(), vfstd=vf.float().std().item(), ystd=y.float().std().item()))
        lastq, lastvf = torch.randn(x.shape).to(tmodel.device), torch.ones_like(vf).to(tmodel.device)
        tvroffset=gv["tmdelta"]*gv["tvrdeltas"] 
        if self.calctop:
            if tidx-tvroffset in self.xx:
                lastq, lastvf=self.xx[tidx-tvroffset], self.vfs[tidx-tvroffset]
            infos = tmodel.calcfee(tidx, 
                    x.to(tmodel.device), y.to(tmodel.device), vf.to(tmodel.device), lastq.to(tmodel.device), lastvf.to(tmodel.device), self.calctop)
            for xoff in range(infos["ret"].shape[0]):
                prefix="task_"+str(xoff)+"_"
                for key in infos:
                    adddict(df, prefix, xoff, [key], [infos[key]])
        
        df=pd.DataFrame(df)
        flog("day:",tidx, gv["ts"][tidx], "tidx:",tidx,":"+self.name+" test:", df.to_dict())
        if tidx-tvroffset in self.xx:
            del self.xx[tidx-tvroffset], self.vfs[tidx-tvroffset]
        if tidx-tvroffset in self.yy:
            del self.yy[tidx-tvroffset]
        #torch.cuda.empty_cache()
